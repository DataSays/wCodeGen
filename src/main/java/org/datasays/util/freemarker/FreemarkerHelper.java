...
public class FreemarkerHelper {
	...
	public void process(String ftlName, Map<String, Object> model, Writer out) {
		Template tpl;
		try {
			tpl = cfg.getTemplate(ftlName, Encoding);
			if (model == null) {
	...
	public String processObj(String ftlName, Object model) {
		StringWriter sw = new StringWriter();
		this.processObj(ftlName, model, sw);
		return sw.toString();
	}

	public void processObj(String ftlName, Object model, Writer out) {
		Template tpl;
		try {
			tpl = cfg.getTemplate(ftlName, Encoding);
			if (model == null) {
	...
	public void processObj(String ftlName, Object model, String targetFile) {
		try {
			LOG.debug(ftlName + "->" + targetFile);
			Writer out = new StringWriter();
			processObj(ftlName, model, out);
			File target = new File(targetFile);
			if (!target.exists()) {
				FileUtil.mkdirs(target.getParentFile());
				target.createNewFile();
