...
public class FtlCodeGen {
	...
	//监视目录文件改动
	public void watch() {
		LOG.info("watching " + "*" + Ftl + " on " + sourceDir.getAbsolutePath());
		DirWatcher dirWatcher = new DirWatcher(sourceDir.getAbsolutePath());
		dirWatcher.monitor("*" + Ftl);
		dirWatcher.register(new DirWatcherListener() {
			public void onChange(File file, DirWatcher.Event event) {
				try {
					String fileName = getRelativePath(file.getAbsolutePath());
					if (event.compareTo(DirWatcher.Event.DELETED) == 0) {
						fileName = targetDir.getAbsolutePath() + fileName;
						...
						gen(fileName);
						LOG.debug("create/update:" + fileName);
					}
				} catch (IOException e) {
					LOG.error(e.getMessage(), e);
				}
		...
		dirWatcher.start(PollingInterval);
	}

	public void dispatch(String cmd) {
		if ("watch".equalsIgnoreCase(cmd) || "wt".equalsIgnoreCase(cmd)) {
			watch();
		} else if ("updateAll".equalsIgnoreCase(cmd) || "up".equalsIgnoreCase(cmd)) {
			updateAll();
	...
	public static void main(String[] args) {
		...
		System.out.println("watch wt    :watch source dir and while the file changed then re-gen it.");
