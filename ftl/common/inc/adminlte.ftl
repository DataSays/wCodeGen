...
<!DOCTYPE html>
<html ms-controller="documentBox" ms-click="closeKeyboard">
<head>
    <meta charset="utf-8">
...
<script src="/static/adminlte/plugins/sparkline/jquery.sparkline.min.js"></script>
    ...
    <!-- mustache -->
		...
		<#if !deps?seq_contains('no_inc_select2')>
		<!-- select2 -->
...
<#macro pagination page formId='searchForm'>
			...
			<@LoadJs>
				function page(n,s){
					$("#${formId}").append("<input id='pageNo' name='pageNo' type='hidden' value='${page.pageNo}'/><input id='pageSize' name='pageSize' type='hidden' value='${page.pageSize}'/>");
					$("#pageNo").val(n);
					$("#pageSize").val(s);
					$("#${formId}").submit();
			        return false;
...
<#-- 列表中的操作按钮 -->
<#macro actionButton icon='default' onClick=''>
<button type="button" class="btn btn-xs btn-${icon}" ${c.htmlAttr('onClick', onClick)}><#nested></button>
</#macro>
...
---------->
 <#macro dateRanges id label col=4 timeStart='' timeEnd='' type='date'>
<div class="${autoCol(col)}">
	<div class="form-group">
		<label for="${id}" class="col-xs-4 col-sm-3 control-label">${label}</label>
		<div class="col-xs-8 col-sm-9">
	...
	<div class="form-group">
		...
		<div class="col-xs-8 col-sm-9">
		  <div class="input-group" style="width:100%">
		    <button type="button" class="btn btn-default pull-right" id="${id}" style="width:100%;text-align:left;">
...
<#macro box id='' cls='primary' ext=''>
	<div <#if id != ''>id="${id}"</#if> class="box box-${cls}" ${ext}><#nested></div>
</#macro>

<#macro boxHeader title=''  subTitle='' id='' cls='' ext=''  icon='' >
		<div class="box-header ${cls}" <#if id != ''>${id}</#if>  ${ext}>
...
<#macro boxBody id='' cls='' ext=''>
	<div class="box-body ${cls}" <#if id != ''>${id}</#if>  ${ext}>
		<#nested>
	</div>
</#macro>

<#macro boxFooter  id='' cls='' ext=''>
	<div class="box-footer ${cls}" <#if id != ''>${id}</#if>  ${ext}>
		<#nested>
	</div>
</#macro>
